...
#abort Target.all.inspect
  if Target.where(name: $targetname).count == 0
    #abort "fillFormForNewCustomer: target with name=#{$targetname} does not exist"
...
targetsolutionList.each do |targetsolution|
   ...
   describe "On target solution '#{targetsolution}'" do  
     before do
        Target.last.destroy!
        p Target.all.inspect
        FactoryGirl.create("target_#{targetsolution}".to_sym).should be_valid
        #Target.all.count.should be(1)
p Target.all.count.inspect
        $myTarget = Target.last
        $myTarget.update_attributes!(configuration: $myTarget.configuration + $FPAFOmit)
        $targetname = targetsolution
        $target = $myTarget.configuration
        $myTarget.destroy!
p Target.all.count.inspect
     end
     after(:all) do
       # clean the target system:

       # TODO: creates an abort with message 
       #       NoMethodError: undefined method `gsub' for nil:NilClass
       #       occurred at /home/provisioningengine/ProvisioningEnginev0.4.6.6/app/models/provisioningobject.rb:66:in `provision'
       Delayed::Worker.delay_jobs = true
       createObject("Customer") unless Customer.where(name: $customerName).count > 0
p $customerName.inspect
p Customer.where(name: $customerName).count.inspect
p "$targetname = " + $targetname.inspect
#p createObject("Customer").inspect
       #createObject("Site")
       #createObject("User")
       Delayed::Worker.delay_jobs = false
       destroyObjectByNameRecursive("Customer")
     end # after(:all) do
     it "kehrgiselru" do
     end
   ...
   #abort page.html.gsub(/[\n\t]/, '')
       ...
       describe "Destroy #{obj}" do
         describe "Delete #{obj} from database" do
           before {
             ...
             case obj
               when /Customer/
	         createObject(obj) unless Object.const_get(myObject(obj)).where(name: $customerName ).count > 0
                 # needed for the case the object existed already in the database:
                 visit provisioningobject_path(Object.const_get(myObject(obj)).where(name: $customerName ).last) if obj == "Customer"
               when /Site/
	         createObject(obj) unless Object.const_get(myObject(obj)).where(name: "Example#{obj}" ).count > 0
                 # needed for the case the object existed already in the database:
                 visit provisioningobject_path(Object.const_get(myObject(obj)).where(name: "Example#{obj}" ).last) if obj == "Site"
               when /User/
	         createObject(obj) unless Object.const_get(myObject(obj)).where(Extension: "30800" ).count > 0
                 # needed for the case the object existed already in the database:
                 visit provisioningobject_path(Object.const_get(myObject(obj)).where(Extension: "30800" ).last) if obj == "User"
               else
                 abort "unkown object type"
             end
	   ...
	   let(:submit) { "Delete #{obj}" }
           ...
           it "should delete an #{obj} from the database, if the status contains 'was already de-provisioned'" do
             ...
             #p page.html.gsub(/[\n\t]/, '')
	     #abort Object.const_get(myObject(obj)).where(name: $customerName ).last.id.inspect if obj == "Customer"
   ...
   objectList.each do |obj|  # 2nd time
     describe "On target solution '#{targetsolution}'" do
       describe "index" do # 2nd time
         before(:each) do
           #createProvisioningDB_manual
   ...
   #p page.html.gsub(/[\n\t]/, '')
       ...
       end # describe "index" do # 2nd time
     end # objectList.each do |obj|  # 2nd time
   end # describe "On target solution '#{targetsolution}'" do
   ...
   end # describe "Customer can be de-provisioned, even if a manually added site is present" do
