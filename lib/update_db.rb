class UpdateDB
  ...
  def perform(targetobject) #, recursive=true)
    ...
    if responseBody.is_a?(Fixnum)
      raise "ERROR: wrong responseBody type (Fixnum (#{responseBody}) instead of String)"
    end
    
    # only Customer, Site, User are supported:
    raise "lib/update_db.rb.perform(targetobject): Unsupported class" unless targetobject.is_a?(Customer) || targetobject.is_a?(Site) || targetobject.is_a?(User) || targetobject == Customer || targetobject == Site || targetobject == User
#raise (targetobject == Customer).inspect
...
#raise responseBody[/ERROR.*$/]
    ...
    if targetobject.is_a?(Target)
      raise "synchronization of Targets is not supported (yet)"      
    elsif targetobject.is_a?(Customer) || targetobject == Customer
	#p xml_data.inspect
	#raise doc.root.elements["GetBGListData"].elements["BGName"].inspect
		#raise targetobject.inspect
      if targetobject.name.match(/_sync_dummyCustomer_________________/) || targetobject == Customer #targetobject.id.nil? 
		...
		#raise Site.where(name: element.elements["SiteName"].text, customer: targetobject.customer).inspect
...
#raise "verified existence"
      ...
      if targetobject.id.nil? 
		...
		#raise xml_data.inspect
		#raise targetobject.id.inspect
		#raise doc.root.elements["Sites"].inspect
        # assume that this is a dummy targetobject that has been created to synchronize all instances from the target system to the DB
...
#            targetobject.update_attribute('extensionlength', element.elements["ExtensionLength"].text )
      ...
      if targetobject.id.nil?
		...
		#raise xml_data.inspect
		#raise targetobject.id.inspect
		#raise doc.root.elements.inspect
        # assume that this is a dummy targetobject that has been created to synchronize all instances from the target system to the DB
        # TODO: create provisioningobjects that are on the target system and not in the DB
		...
		#raise /\A#{targetobject.site.countrycode}#{targetobject.site.areacode}#{targetobject.site.localofficecode}/.inspect
		...
		#raise User.where(extension: this_extension,  site: targetobject.site).inspect
